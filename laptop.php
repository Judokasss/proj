...
if (isset($_POST['editor'])) {
    $sql = ("UPDATE laptop SET `product_name` = ?, `product_price` = ?, `product_image` = ?, `product_images` = ?, `description` = ? WHERE `id` = ? ");
...
if (isset($_POST['addtocart'])) {
    ...
    if (in_array($avatar_ext, $allowed_exts) && $avatar_error === 0 && $avatar_size <= 5242880) { // validate avatar file
        ...
        $sql = "INSERT INTO laptop (product_name, product_image, product_price, product_images, description) VALUES (?, ?, ?, ?, ?)";
        $query = $dbh->prepare($sql);
